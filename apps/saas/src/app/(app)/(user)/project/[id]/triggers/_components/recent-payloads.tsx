    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    View recent webhook payloads to understand available data for
                    conditions and prompts. Click copy to use in your template.
                </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                {payloads.map((event) => (
                    <div key={event.id} className="bg-muted rounded-md border">
                        <div className="border-b px-3 py-2 flex items-center justify-between">
                            <span className="text-muted-foreground text-xs">
                                {new Date(event.receivedAt).toLocaleString()}
                            </span>
                            <CopyButton
                                text={JSON.stringify(event.rawPayload, null, 2)}
                                label="Copy payload"
                            />
                        </div>
